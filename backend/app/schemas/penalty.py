...
class PenaltyCreate(BaseModel):
    ...
    score: float | None = Field(default=None, ge=0)  # if omitted, backend sets from PENALTY_TYPE_SCORES
...
class PenaltyRead(PenaltyCreate):
    id: str
    created_at: datetime
    score: float | None = None  # CRS deduction; from model (may be null for legacy rows)
